...
create policy "orders own" on public.orders for select using(auth.uid() = user_id or exists (select 1 from public.profiles where id = auth.uid() and role = 'ADMIN'));
...
create policy "orders payment" on public.orders for update using(auth.uid() = user_id or exists (select 1 from public.profiles where id = auth.uid() and role = 'ADMIN'));

-- Allow public tracking by order number
...
create policy "files own" on public.order_files for all using(exists(select 1 from public.orders o where o.id = order_id and (o.user_id = auth.uid() or exists (select 1 from public.profiles where id = auth.uid() and role = 'ADMIN'))));

-- Status history policies
drop policy if exists "history own" on public.status_history;
create policy "history own" on public.status_history for all using(true);

-- Storage bucket policies (documents & payment-proofs)
...
create policy "authenticated read documents" on storage.objects for select using (bucket_id = 'documents' and auth.role() = 'authenticated');
...
create policy "authenticated read payment proofs" on storage.objects for select using (bucket_id = 'payment-proofs' and auth.role() = 'authenticated');
